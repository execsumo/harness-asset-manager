...
- **Hermes parity sidecar**: Hermes' canonical index walker follows symlinks, so a HAM binding is discovered and invoked natively. Four Hermes *management* paths bypass that walker (three scan with `Path.rglob`, which never descends a directory symlink; one resolves before matching a categorised name), leaving a bound Skill invisible to the curator, unopenable in the dashboard editor, and subject to a spurious trusted-directory warning. `runtime/hermes_compat.py` closes this by installing a version-stamped shim (`data/hermes/harnessam_hermes_compat.py` plus a `.pth`) into the Hermes virtualenv's `site-packages` on init and on every `start`/`serve` — best-effort and silent, following the `runtime/tailscale.py` posture. It patches lazily via `sys.meta_path` and fails open, so it can never break Hermes. The Hermes source checkout is deliberately untouched: `hermes update` refuses a dirty tree, and a Hermes plugin was ruled out because plugins are not loaded for `hermes curator`. See the README's [Skills parity and the compatibility sidecar](README.md#skills-parity-and-the-compatibility-sidecar) and upstream [hermes-agent#112404](https://github.com/NousResearch/hermes-agent/pull/112404).
...
- **Agent attachments are a derived read model**: The Skills payload includes `agents` per row by inverting the managed agent store's `skills:` frontmatter at read time (`application/agents/attachments.py`). This deliberately uses `AgentStore.scan()` via a late `SkillsQueryService.set_agent_attachments(...)` hook, not `AgentInventoryService`, because inventory reconciliation can write. The seam is best-effort: if the agent store cannot be read, the Skills rows still load without Agent chips.
...
- **Attached Skills**: `skills` is an ordered list of adopted Skill slugs. Agent detail editing validates entries against the managed Skills inventory and offers the adopted Skills as suggestions. Saving an agent auto-enables each newly attached Skill on installed harnesses where that agent is enabled. The Skills page bulk attach/detach endpoint composes with the same agent update path, including validation, audit, Hermes profile provisioning, and auto-enable projection; it does not write agent files directly. Removing a Skill from the list does not disable its harness bindings.
...
**Family-scoped commitments.** A harness-wide tier is too coarse when one family is verified and the rest are not. `HarnessDefinition.family_support_tiers` overrides the harness-wide tier per family, read through `support_tier_for(family)` / `is_core_for(family)`; an absent entry falls back to `support_tier`, so a fully core harness needs no override. Hermes uses this: `support_tier="best_effort"` with `family_support_tiers={"skills": "core"}`. Hermes Skills are verified against a live install and block a release; Hermes slash commands, config bindings, agents and MCP stay best-effort until they earn their own evidence. `core_harness_ids()` with no argument still returns the harness-wide core set, so the narrow promotion cannot leak into unrelated gates.
...
| `yaml` | `~/.hermes/config.yaml`, `<hermes-root>/profiles/<bot>/config.yaml` | `ruamel.yaml` round-trip mode |
