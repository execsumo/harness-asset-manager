...
2. **UI & Filter Consistency**:
   ...
   - Detail sheets and modals (Skills, Agents, Hooks, MCP, Permissions, Slash Commands) include tag viewing, addition, removal with autocomplete suggestions, accessible focus, and header `titleAction` star toggle with full Skills reference parity.
   - Star columns in matrix tables across all taggable families visibly display empty outline stars for unstarred managed items in light and dark themes without requiring hover, with accessible focus rings and descriptive labels.
...
3. **Validation & Testing**:
   ...
   - Frontend component, detail view, matrix view, and cross-family UI pressure test suite (`frontend/src/test/family_tag_star_ui_pressure.test.tsx`) proving 100% parity and zero regressions across all 6 asset families.
