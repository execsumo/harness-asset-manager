...
describe("PermissionDetailSheet", () => {
  ...
  it("calls unmanage API and closes sheet when Remove from HarnessAM is clicked", async () => {
    fetchMock.mockImplementation((input, init) => {
      const url = String(input);
      const method = init?.method || "GET";
      if (url.includes("/api/permissions/fs-read-project/unmanage") && method === "POST") {
        return Promise.resolve(okJson({ ok: true }));
      }
      if (url.includes("/api/permissions/fs-read-project")) {
        return Promise.resolve(okJson(permissionDetailFixture({ kind: "managed" })));
      }
      return Promise.resolve(okJson({}));
    });
    ...
    await waitFor(() => {
      expect(
        fetchMock.mock.calls.some(
          (call) =>
            String(call[0]).includes("/api/permissions/fs-read-project/unmanage") &&
            call[1]?.method === "POST",
        ),
      ).toBe(true);
