...
export const skillsCopy = {
  ...
  "zh-CN": {
    ...
    review: {
      ...
      emptyBody:
        "你的本地 harness 文件夹要么已通过 Harness Asset Manager 使用，要么当前为空。可以从商城安装新的 Skill。",
    },
    filters: {
    ...
    bulk: {
      ...
      confirmDescription: "这会移除 Harness Asset Manager 副本以及所有 harness 中的符号链接。",
      confirmNote: "Harness Asset Manager 存储之外的磁盘来源不会被修改。",
    },
    confirm: {
      removeTitle: "从 Harness Asset Manager 移除此 Skill？",
      removeDescription: (skillName: string) =>
        `这会从 Harness Asset Manager 存储中移除 ${skillName}，并且只为当前已启用的 harness 恢复本地副本。`,
      restoreTo: (labels: readonly string[]) => `将恢复到：${labels.join(", ")}`,
      remove: "移除",
      removing: "移除中",
      deleteTitle: "从 Harness Asset Manager 删除此 Skill？",
      deleteDescription: (skillName: string) =>
    ...
    detail: {
      ...
      harnesses: "Harness",
      locations: "位置",
      storeNote: "Harness Asset Manager Store 是规范的实体包。启用时，工具位置会以符号链接指向它。",
      addToSkillManager: "添加到 Harness Asset Manager",
      managingSkill: "正在管理 Skill",
      deleteSkill: "删除 Skill",
      canonicalPhysicalPackage: "规范实体包",
      symlinkToStore: "指向 Harness Asset Manager Store 的符号链接",
      moreActions: (name: string) => `${name} 的更多操作`,
      removeFromSkillManager: "从 Harness Asset Manager 移除",
      delete: "删除",
      enableOnAll: "全部启用",
