...
const REMEDIATION_MESSAGE = "harness-asset-manager is already installed via Homebrew. Run 'brew uninstall harness-asset-manager' before 'npm install -g @execsumo/harness-asset-manager', or keep using the Homebrew installation.";
...
function detectHomebrewInstall() {
  ...
  const brewList = spawnSync("brew", ["list", "--versions", "harness-asset-manager"], { encoding: "utf8" });
  ...
  for (const prefix of prefixes) {
    ...
    if (fs.existsSync(path.join(prefix, "opt", "harness-asset-manager"))) {
      return true;
    }
    if (isNonEmptyDirectory(path.join(prefix, "Cellar", "harness-asset-manager"))) {
      return true;
    }
  }
  return false;
