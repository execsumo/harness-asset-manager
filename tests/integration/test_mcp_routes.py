...
class McpRoutesTests(unittest.TestCase):
    ...
    def test_inventory_excludes_disabled_and_undetected_harnesses(self) -> None:
        with AppTestHarness(omit_clis=("cursor-agent", "opencode")) as harness:
            # 1. Undetected harnesses (droid, cursor, opencode without CLI or config) excluded
            payload = harness.get_json("/api/mcp/servers")
            cols = [col["harness"] for col in payload["columns"]]
            self.assertIn("claude", cols)
            self.assertIn("codex", cols)
            ...
            (harness.spec.home / ".cursor" / "mcp.json").write_text('{"mcpServers":{}}', encoding="utf-8")
            harness.container.mcp_read_models.invalidate()

            payload_with_cursor = harness.get_json("/api/mcp/servers")
            ...
            payload_disabled = harness.get_json("/api/mcp/servers")
            ...
            _seed_manual_remote(harness, "remote")
            detail = harness.get_json("/api/mcp/servers/remote")
